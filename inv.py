...
def cargar_historial_dataframe():
    """Lee la pestaña 'long' de Google Sheets en tiempo real (ttl=0)."""
    try:
        df = conn.read(worksheet="long", ttl=0)
        if df is None or df.empty:
            return pd.DataFrame(columns=["USUARIO", "ACCION", "PRODUCTO", "CANTIDAD", "FECHA"])
        return df
    except Exception:
        return pd.DataFrame(columns=["USUARIO", "ACCION", "PRODUCTO", "CANTIDAD", "FECHA"])
...
def registrar_en_historial(producto, tipo, cantidad, fecha):
    """Registra los movimientos directamente en la pestaña 'long' con las columnas exactas."""
    try:
        df_historial = conn.read(worksheet="long", ttl=0)
        if df_historial is None or df_historial.empty:
            df_historial = pd.DataFrame(columns=["USUARIO", "ACCION", "PRODUCTO", "CANTIDAD", "FECHA"])
    except Exception:
        df_historial = pd.DataFrame(columns=["USUARIO", "ACCION", "PRODUCTO", "CANTIDAD", "FECHA"])
    ...
    conn.update(worksheet="long", data=df_historial_actualizado.astype(str))
...
def registrar_producto(nombre, cantidad, fecha, almacen): 
    ...
    # 📝 REGISTRO EN LA PESTAÑA LONG
    registrar_en_historial(nombre_mayus, "Entrada (Inicial)", cant_limpia, fecha)
...
def registrar_movimiento(nombre, tipo, cantidad): 
    ...
    # 📝 REGISTRO EN LA PESTAÑA LONG
    registrar_en_historial(nombre, tipo, cant_mov, fecha_hoy)
            ...
            if not error:
                ...
                for fila in tabla:
                    ...
                    if nueva_cantidad != cant_anterior:
                        ...
                        # 📝 REGISTRO EN LA PESTAÑA LONG POR AJUSTE MANUAL
                        registrar_en_historial(nuevo_nombre, tipo_mov, abs(diferencia), fecha_hoy)
...
# --------HISTORIAL--------
    ...
    st.subheader("Historial de Movimientos (Pestaña long)")
    df_historial = cargar_historial_dataframe()

    if df_historial.empty:
        st.info("No hay movimientos registrados en la pestaña long.")
    else:
        st.dataframe(
            df_historial,
            column_config={
        ...
        st.download_button("📊 Descargar Historial Excel", exportar_excel_general(df_historial, "long"), "historial_long.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
