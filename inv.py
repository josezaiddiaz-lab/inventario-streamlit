...
if menu == "Registrar Producto": 
    ...
    nombre = st.text_input("Nombre del producto", key="nombre") 
    cantidad = st.number_input("Cantidad inicial", min_value=0, step=1, key="cantidad") 
    almacen = st.selectbox("Almacén", [1, 2, 3, 4, 5, 6], key="almacen")
        ...
        if tabla != lista:
            ...
            if not error:
                for fila in tabla:
                    ...
                    cantidad_anterior = limpiar_entero(inventario[original].get("cantidad", 0))

                    nuevo[nuevo_nombre] = inventario[original]
                    nuevo[nuevo_nombre]["almacen"] = fila["ALMACÉN"]
                    nuevo[nuevo_nombre]["cantidad"] = nueva_cantidad

                    if nueva_cantidad != cantidad_anterior:
                        diferencia = nueva_cantidad - cantidad_anterior
                        tipo = "Ajuste +" if diferencia > 0 else "Ajuste -"

                        nuevo[nuevo_nombre]["movimientos"].append({
                            "tipo": tipo,
...
# --------HISTORIAL--------
    ...
    for nombre, datos in inventario.items():
        for mov in datos.get("movimientos", []):
            tipo_original = mov.get("tipo", "-")
            
            # Buscamos si el tipo original necesita traducirse, si no, dejamos el original
            tipo_limpio = TRADUCCION_TIPOS.get(tipo_original, tipo_original)

            historial.append({
                "PRODUCTO": nombre,
                "TIPO": tipo_limpio, # <-- Usamos el texto limpio aquí
