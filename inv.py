...
# --------MOVIMIENTOS --------
    ...
    # Inicializar variables de estado para limpiar los inputs y el selectbox correctamente
    ...
    if df_productos.empty: 
        ...
        # Callback para resetear el selectbox y limpiar el estado del producto seleccionado
        def limpiar_seleccion_producto():
            st.session_state.select_mov_prod = None
            st.session_state.mov_producto = None

        # Selectbox vinculado con control de índice seguro para limpiar tras el registro
        indice_actual = productos.index(st.session_state.mov_producto) if st.session_state.mov_producto in productos else 0
        
        nombre = st.selectbox(
            "Producto", 
            ["Selecciona un producto..."] + productos, 
            index=0 if st.session_state.mov_producto is None else productos.index(st.session_state.mov_producto) + 1,
            key="select_mov_prod"
        ) 
        
        # Sincronizar selección real
            ...
            if not nombre:
                ...
                if tipo_mov == "Salida":
                    if cant_mov > cant_total_actual or abiertos_mov > cant_abiertos_actual: 
                        ...
                        # Limpiar selección y fijar mensaje de éxito
                        st.session_state.mov_producto = None
