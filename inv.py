...
with col_top1:
    ...
    with c_btn1:
        if st.button("🔄 Actualizar"):
            st.cache_data.clear()
            st.session_state["menu_opcion"] = "Registrar Producto"
            st.rerun()
    with c_btn2:
        if st.button("🚪 Salir"):
            st.session_state["autenticado"] = False
...
menu = st.selectbox(
    ...
    ["Registrar Producto", "Registrar Movimiento", "Ver Inventario", "Visualizar Producto", "Historial de Movimientos", "Solicitar Pedidos"],
    key="menu_opcion"
)
...
def cargar_inventario_dataframe(): 
    try:
        ...
        if df is None or df.empty:
            return pd.DataFrame(columns=["PRODUCTO", "CANTIDAD TOTAL", "CANTIDAD ABIERTOS", "ALMACEN", "FECHA INGRESO", "OBSERVACION", "IMAGEN 1", "IMAGEN 2"])
        ...
        if "IMAGEN 1" not in df.columns:
            ...
            df["IMAGEN 1"] = df["IMAGEN 1"].fillna("").astype(str)
        if "IMAGEN 2" not in df.columns:
            df["IMAGEN 2"] = ""
        else:
            df["IMAGEN 2"] = df["IMAGEN 2"].fillna("").astype(str)
        ...
        st.error(f"❌ Error al conectar con Google Sheets (Productos): {e}")
        return pd.DataFrame(columns=["PRODUCTO", "CANTIDAD TOTAL", "CANTIDAD ABIERTOS", "ALMACEN", "FECHA INGRESO", "OBSERVACION", "IMAGEN 1", "IMAGEN 2"])
...
def cargar_historial_dataframe():
    try:
        ...
        st.error(f"❌ Error al conectar con Google Sheets (Historial): {e}")
        return pd.DataFrame(columns=["PRODUCTO", "TIPO", "CANTIDAD", "ABIERTOS", "FECHA", "DESCRIPCION"])
...
if menu == "Registrar Producto": 
    ...
    if "observacion" not in st.session_state: st.session_state.observacion = ""
    ...
    def procesar_y_limpiar():
        ...
        obs = st.session_state.observacion
    ...
    st.text_input("Observación", key="observacion")
...
# --------MOVIMIENTOS --------
    ...
    # 1. Asegurar que las variables de estado existan
    if "select_mov_prod" not in st.session_state: st.session_state.select_mov_prod = "Selecciona un producto..."
    if "input_cant_mov" not in st.session_state: st.session_state.input_cant_mov = 1
    if "input_abierta_mov" not in st.session_state: st.session_state.input_abierta_mov = 0
    if "input_desc_mov" not in st.session_state: st.session_state.input_desc_mov = ""
    if "mensaje_exito_mov" not in st.session_state: st.session_state.mensaje_exito_mov = ""
    
    # Bandera para limpiar el selectbox de forma segura antes de renderizarlo
    if st.session_state.get("limpiar_selectbox_pend", False):
        st.session_state.select_mov_prod = "Selecciona un producto..."
        st.session_state.limpiar_selectbox_pend = False
    ...
    if df_productos.empty: 
        ...
        opciones_select = ["Selecciona un producto..."] + productos
        if st.session_state.select_mov_prod not in opciones_select:
            st.session_state.select_mov_prod = "Selecciona un producto..."
        ...
        prod_seleccionado = None if nombre == "Selecciona un producto..." else nombre

        tipo_mov = st.radio("Tipo", ["Entrada", "Salida"], horizontal=True, key="radio_tipo_mov")
            
        cantidad = st.number_input("Cantidad total a mover", min_value=1, step=1, key="input_cant_mov") 
        abiertos_afectados = st.number_input("¿Cuántos de estos son abiertos y en buen estado?", min_value=0, max_value=int(cantidad), step=1, key="input_abierta_mov")
        descripcion = st.text_input("Descripción (Opcional)", placeholder="Escribe un detalle o motivo opcional...", key="input_desc_mov")
        
        if st.button("Registrar Movimiento"): 
            if not prod_seleccionado:
                st.warning("Por favor, selecciona un producto.")
                st.session_state.mensaje_exito_mov = ""
            else:
                ...
                if tipo_mov == "Salida":
                    if cant_mov > cant_total_actual or abiertos_mov > cant_abiertos_actual: 
                        ...
                        registrar_en_historial(prod_seleccionado, tipo_mov, cant_mov, abiertos_mov, fecha_hoy, f"{descripcion} (Abiertos: {abiertos_mov})")
                        usuario_actual = st.session_state.get("usuario", "sistema")
                        registrar_en_hoja_long(usuario_actual, tipo_mov, prod_seleccionado, cant_mov, fecha_hoy)
                        
                        # Activamos la bandera para limpiar el selectbox de forma segura antes de renderizar
                        st.session_state.limpiar_selectbox_pend = True
                        st.session_state.mensaje_exito_mov = f"✅ Movimiento de salida registrado con éxito para el producto: {prod_seleccionado}"
                        st.rerun()
                else: 
                    nueva_total = cant_total_actual + cant_mov
                    nuevo_abiertos = cant_abiertos_actual + abiertos_mov
                    ...
                    # Activamos la bandera para limpiar el selectbox de forma segura antes de renderizar
                    st.session_state.limpiar_selectbox_pend = True
                    st.session_state.mensaje_exito_mov = f"✅ Movimiento de entrada registrado con éxito para el producto: {prod_seleccionado}"
                    st.rerun()

        # Mostrar el mensaje de éxito abajo del botón de manera persistente tras la recarga
        if st.session_state.mensaje_exito_mov:
            st.success(st.session_state.mensaje_exito_mov)
            st.session_state.mensaje_exito_mov = ""

# --------VER INVENTARIO--------
elif menu == "Ver Inventario": 
    st.subheader("Reporte General de Inventario") 
    st.caption("Puedes corregir nombre, cantidades, almacén, fecha de ingreso y observaciones.")
    ...
    if df_productos.empty: 
        ...
        class PDFReporte(FPDF):
            def __init__(self):
                super().__init__()
                self.col_widths = [50, 20, 20, 20, 35, 45]
            ...
            def header(self):
                ...
                headers = ["PRODUCTO", "TOTAL", "ABIERTOS", "ALMACEN", "FECHA INGRESO", "OBSERVACION"]
            ...
            def add_row_multi_cell(self, row_data):
                ...
                obs_text = limpiar_texto_pdf(row_data[5])
                
                w_prod = self.col_widths[0]
                w_obs = self.col_widths[5]
                line_height = 5
                
                def obtener_lineas(texto, ancho):
                    words = texto.split(' ')
                    lines = []
                    current_line = ""
                    for word in words:
                        if self.get_string_width(word) > (ancho - 4):
                            if current_line:
                                lines.append(current_line)
                                current_line = ""
                            sub_word = ""
                            for char in word:
                                if self.get_string_width(sub_word + char) <= (ancho - 4):
                                    sub_word += char
                                else:
                            ...
                            if self.get_string_width(test_line) <= (ancho - 4):
                ...
                lines_prod = obtener_lineas(prod_text, w_prod)
                lines_obs = obtener_lineas(obs_text, w_obs)
                
                num_lines = max(len(lines_prod), len(lines_obs), 1)
                ...
                for i, w in enumerate(self.col_widths):
                    self.rect(x_start + sum(self.col_widths[:i]), y_start, w, row_height)

                self.set_xy(x_start, y_start + (row_height - (len(lines_prod) * line_height)) / 2)
                ...
                self.set_xy(x_start + sum(self.col_widths[:5]), y_start + (row_height - (len(lines_obs) * line_height)) / 2)
                for line in lines_obs:
                    self.cell(w_obs, line_height, line, 0, 0, 'L')
                    self.set_xy(x_start + sum(self.col_widths[:5]), self.get_y() + line_height)
        ...
        def exportar_pdf(): 
            ...
            for _, row in df_productos.iterrows(): 
                ...
                pdf.add_row_multi_cell([prod_nombre, tot, ab, almacen, fecha_ing, obs])
...
# --------VISUALIZAR PRODUCTO--------
    ...
    if df_productos.empty:
        ...
        productos = sorted(df_productos["PRODUCTO"].astype(str).tolist())
        prod_seleccionado = st.selectbox("Selecciona un producto...", productos, index=None, placeholder="Selecciona un producto...")
        
        if prod_seleccionado:
            row = df_productos[df_productos["PRODUCTO"] == prod_seleccionado].iloc[0]
            
            col_info, col_img = st.columns(2)
            
            with col_info:
                st.markdown(f"### 📦 {row['PRODUCTO']}")
                st.write(f"**Cantidad Total:** {limpiar_entero(row['CANTIDAD TOTAL'])}")
                st.write(f"**Cantidad Abiertos:** {limpiar_entero(row['CANTIDAD ABIERTOS'])}")
                st.write(f"**Almacén:** {row['ALMACEN']}")
                st.write(f"**Fecha de Ingreso:** {row.get('FECHA INGRESO', '-')}")
            ...
            with col_img:
                ...
                def comprimir_imagen_a_base64(archivo_subido):
                    try:
                        imagen = Image.open(archivo_subido)
                        imagen.thumbnail((400, 400))
                        buffered = io.BytesIO()
                        imagen.save(buffered, format="JPEG", quality=70)
                ...
                archivo_foto_1 = st.file_uploader("Sube o toma foto para Imagen 1", type=["jpg", "jpeg", "png"], key="subir_img1")
                nueva_url_1 = img_url_1
                if archivo_foto_1 is not None:
                    resultado_comprimido = comprimir_imagen_a_base64(archivo_foto_1)
                    if resultado_comprimido:
                        nueva_url_1 = resultado_comprimido

                archivo_foto_2 = st.file_uploader("Sube o toma foto para Imagen 2", type=["jpg", "jpeg", "png"], key="subir_img2")
                nueva_url_2 = img_url_2
                if archivo_foto_2 is not None:
                    resultado_comprimido_2 = comprimir_imagen_a_base64(archivo_foto_2)
                    if resultado_comprimido_2:
                        nueva_url_2 = resultado_comprimido_2
