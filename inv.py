...
# --------EXPORTAR REPORTE GENERAL A PDF (CON CENTRADO)-------- 
def exportar_pdf(): 
    df_productos = cargar_inventario_dataframe()
    ...
    for _, row in df_productos.iterrows(): 
        ...
        pdf.cell(60, 10, prod_nombre, 1, 0, 'L') 
        pdf.cell(30, 10, cant, 1, 0, 'C') 
        pdf.cell(30, 10, almacen, 1, 0, 'C') 
        pdf.cell(50, 10, fecha_ing, 1, 1, 'C') 
        
    return bytes(pdf.output(dest='S'))

# --------EXPORTAR REPORTE DE PEDIDOS A PDF (CON CENTRADO)-------- 
def exportar_pdf_pedidos(): 
    ...
    for _, row in df_productos.iterrows(): 
        ...
        if cant <= 1: 
            pdf.cell(80, 10, limpiar_texto_pdf(row["PRODUCTO"]), 1, 0, 'L') 
            pdf.cell(40, 10, str(cant), 1, 0, 'C') 
            pdf.cell(40, 10, str(row["ALMACEN"]), 1, 1, 'C') 
            
    return bytes(pdf.output(dest='S'))

# --------EXPORTAR A EXCEL (CON CELDAS CENTRADAS)-------- 
def exportar_excel_general(df, titulo_pestana="Inventario"):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # Dar formato centrado a columnas específicas
        ...
        for col_idx, col_name in enumerate(df.columns, 1):
            ...
            # Recorrer filas para alinear
            for row_idx in range(2, len(df) + 2):
                cell = worksheet[f"{cell_letter}{row_idx}"]
                cell.border = thin_border
                cell.font = Font(name="Arial", size=10)
                
                # Centrar Cantidad, Almacen, Fecha o Tipo si aplican
...
# --------PEDIDOS--------
    ...
    if not bajos:
        ...
        # Muestra la tabla de pedidos centrando CANTIDAD y ALMACÉN sin decimales
        st.dataframe(
            df_bajos,
            column_config={
                "PRODUCTO": st.column_config.TextColumn(),
