...
if menu == "Registrar Producto": 
    ...
    # 2. Definir la función Callback que procesa y limpia los inputs
    def procesar_y_limpiar():
        ...
        if nom.strip():
            ...
            if exito:
                # Limpiamos las variables DENTRO del callback
                st.session_state.nombre = ""
                st.session_state.cantidad = 0
                st.session_state.almacen = 1
                st.toast("✅ Producto registrado correctamente en Google Sheets.")
        else:
