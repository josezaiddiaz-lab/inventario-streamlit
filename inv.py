...
def cargar_inventario_dataframe(): 
    ...
    try:
        ...
        # Asegurar que exista la columna ABIERTOS para bases de datos antiguas
...
def registrar_producto(nombre, cantidad, fecha, almacen): 
    ...
    registrar_en_historial(nombre_mayus, "Entrada (Inicial) - Cerrado", cant_limpia, fecha, "Registro inicial de producto")
    
    usuario_actual = st.session_state.get("usuario", "sistema")
    registrar_en_hoja_long(usuario_actual, "Entrada (Inicial) - Cerrado", nombre_mayus, cant_limpia, fecha)
    
    return True

# ----------MOVIMIENTO DE STOCK---------- 
def registrar_movimiento(nombre, tipo_movimiento, estado, cantidad, descripcion=""): 
    df_productos = cargar_inventario_dataframe()
    ...
    if tipo_movimiento == "Salida" and cant_mov > cant_actual: 
        st.warning("No hay suficiente stock para realizar la salida.") 
        return 
        
    if tipo_movimiento == "Entrada":
        cant_actual += cant_mov
        if estado == "Abierto":
            abiertos_actual += cant_mov
    else: 
        cant_actual -= cant_mov
        if estado == "Abierto":
            abiertos_actual = max(0, abiertos_actual - cant_mov)
        
    df_productos.loc[idx, "CANTIDAD_ACTUAL"] = cant_actual
    df_productos.loc[idx, "ABIERTOS"] = abiertos_actual
    conn.update(worksheet="Productos", data=df_productos.astype(str))
    
    fecha_hoy = datetime.now().strftime('%Y-%m-%d')
    tipo_completo = f"{tipo_movimiento} - {estado}"
    
    registrar_en_historial(nombre, tipo_completo, cant_mov, fecha_hoy, descripcion)
...
if menu == "Registrar Producto": 
    ...
    if "cantidad" not in st.session_state: st.session_state.cantidad = 0
...
# --------MOVIMIENTOS--------
    ...
    if df_productos.empty: 
        ...
        if st.button("Registrar Movimiento"): 
            registrar_movimiento(nombre, tipo_mov, estado, cantidad, descripcion)
            st.rerun()
    ...
    if df_productos.empty: 
        ...
        for _, row in df_productos.iterrows(): 
            ...
            lista.append({ 
                ...
                "PRODUCTO": nombre, 
                "CANTIDAD_ACTUAL": limpiar_entero(row["CANTIDAD_ACTUAL"]),
        ...
        tabla = st.data_editor(
            ...
            column_config={
                ...
                "CANTIDAD_ACTUAL": st.column_config.NumberColumn(label="CANTIDAD", alignment="center", format="%d"),
                "ABIERTOS": st.column_config.NumberColumn(label="ABIERTOS", alignment="center", min_value=0, step=1, format="%d"),
        ...
        if tabla != lista:
            ...
            if not error:
                ...
                for fila in tabla:
                    nuevo_nombre = fila["PRODUCTO"].strip().upper()
                    nueva_cantidad = limpiar_entero(fila["CANTIDAD_ACTUAL"])
