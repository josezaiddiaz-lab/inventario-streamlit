...
menu = st.sidebar.selectbox(
    ...
    ["Registrar Producto", "Registrar Movimiento", "Ver Inventario", "Historial"]
)
    ...
    if len(inventario) == 0:
        ...
        for prod, datos in inventario.items():
            lista.append({
                ...
                "ULTIMO MOVIMIENTO": datos["movimientos"][-1]["fecha"] if datos["movimientos"] else ""
            })
        ...
        if st.button("Guardar cambios"):
            ...
            for fila in tabla:
                ...
                nuevo[nombre] = {
                    ...
                    "movimientos": inventario[fila["Producto Original"]]["movimientos"]
                }
...
# --------HISTORIAL--------
    ...
    if not historial:
        ...
        data_json = json.dumps(inventario, indent=4, ensure_ascii=False)

        st.download_button(
            label="💾 Descargar Respaldo (JSON)",
            data=data_json,
            file_name="respaldo_inventario.json",
