...
# --------MOVIMIENTOS (MODIFICADO)--------
    ...
    # Inicializar variables de estado para limpiar los inputs y mostrar mensaje abajo
    if "mov_producto" not in st.session_state: st.session_state.mov_producto = None
    if "mov_cantidad" not in st.session_state: st.session_state.mov_cantidad = 1
    if "mov_abiertos" not in st.session_state: st.session_state.mov_abiertos = 0
    if "mov_desc" not in st.session_state: st.session_state.mov_desc = ""
    ...
    if df_productos.empty: 
        ...
        # Selectbox y campos vinculados al session_state para poder limpiarlos
        nombre = st.selectbox("Producto", productos, index=productos.index(st.session_state.mov_producto) if st.session_state.mov_producto in productos else None, placeholder="Selecciona un producto...", key="select_mov_prod") 
            ...
            if not nombre:
                ...
                if tipo_mov == "Salida":
                    if cant_mov > cant_total_actual or abiertos_mov > cant_abiertos_actual: 
                        ...
                        # Limpiar campos reseteando estados y fijar mensaje de éxito abajo
                        st.session_state.mov_producto = None
                        st.session_state.mensaje_exito_mov = f"✅ Movimiento de salida registrado con éxito para el producto: {nombre}"
                        st.rerun()
                else: 
                    nueva_total = cant_total_actual + cant_mov
                    nuevo_abiertos = cant_abiertos_actual + abiertos_mov
                    ...
                    # Limpiar campos reseteando estados y fijar mensaje de éxito abajo
                    st.session_state.mov_producto = None
                    st.session_state.mensaje_exito_mov = f"✅ Movimiento de entrada registrado con éxito para el producto: {nombre}"
                    st.rerun()
        ...
        if st.session_state.mensaje_exito_mov:
            ...
            # Limpiar el mensaje para que no aparezca eternamente si cambia de pestaña o realiza otra acción
            st.session_state.mensaje_exito_mov = ""
