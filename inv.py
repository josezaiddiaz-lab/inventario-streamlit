...
# -------REGISTRAR MOVIMIENTO (BUSCADOR INTELIGENTE ACTIVADO)----------- 
if menu == "Registrar Movimiento": 
    st.subheader("Registrar Movimiento de Stock") 
    inventario = cargar_inventario() 
    
    # Ordenamos alfabéticamente la lista para facilitar la lectura visual
    productos = sorted(list(inventario.keys())) 
    
    if not productos: 
        st.info("No hay productos registrados.") 
    else: 
        # NOTA: st.selectbox incluye por defecto barra de búsqueda. Al escribir texto (ej. 'TN')
        # filtrará automáticamente de forma predictiva en tiempo real.
        nombre = st.selectbox(
            "Selecciona o busca el producto", 
            options=productos,
            index=None,  # Inicia vacío para forzar al usuario a buscar o elegir voluntariamente
