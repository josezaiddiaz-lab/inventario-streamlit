...
if menu == "Ver Inventario": 
    ...
    if not inventario: 
        ...
        # Corrección aplicada en 'column_config' para bloquear Cantidad y Último Movimiento
        tabla_editada = st.data_editor(
            lista_productos, 
            column_config={
                "Producto Original": None, 
                "Cantidad Disponible": st.column_config.NumberColumn(disabled=True), # <-- BLOQUEADO
                "Último Movimiento": st.column_config.TextColumn(disabled=True)       # <-- BLOQUEADO
            },
            hide_index=True,
