...
def registrar_producto(nombre, cantidad, fecha): 
    ...
    # Forzamos que la cantidad inicial sea un número entero
...
def registrar_movimiento(nombre, tipo, cantidad): 
    ...
    # Aseguramos que la cantidad actual e ingresada se traten como números
    cantidad_actual = int(inventario[nombre]["cantidad"])
    cantidad_mov = int(cantidad)
...
def exportar_pdf_pedidos(): 
    ...
    pdf.set_font("Arial", '', 10) 
    for nombre, datos in inventario.items(): 
        # CORRECCIÓN: Forzamos la lectura como entero para asegurar compatibilidad en el PDF
        if int(datos["cantidad"]) <= 1: 
            nombre_limpio = nombre.encode('latin-1', 'replace').decode('latin-1')
            pdf.cell(100, 10, nombre_limpio, 1) 
            pdf.cell(50, 10, str(datos["cantidad"]), 1) 
            pdf.ln() 
    ...
    if not inventario: 
        ...
        for nombre, datos in inventario.items(): 
            ...
            lista_productos.append({ 
                ...
                "Producto": nombre, 
                # Forzamos int aquí para blindar la tabla contra textos ocultos
                "Cantidad Disponible": int(datos["cantidad"]), 
        ...
        if tabla_editada != lista_productos:
            ...
            if error_duplicado:
                ...
                for fila in tabla_editada:
                    id_original = fila["Producto Original"]
                    nuevo_nombre = fila["Producto"].strip().upper()
                    nueva_cantidad = int(fila["Cantidad Disponible"]) # Guardado seguro como entero
