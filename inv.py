...
def registrar_en_long(usuario, accion, cantidad, producto, fecha):
    """Registra los movimientos exclusivamente en la pestaña 'long' de Google Sheets."""
    try:
        df_long = conn.read(worksheet="long", ttl=0)
        if df_long is None or df_long.empty:
            df_long = pd.DataFrame(columns=["USUARIO", "ACCION", "CANTIDAD", "PRODUCTO", "FECHA"])
    except Exception:
        df_long = pd.DataFrame(columns=["USUARIO", "ACCION", "CANTIDAD", "PRODUCTO", "FECHA"])
...
def registrar_producto(nombre, cantidad, fecha, almacen): 
    ...
    registrar_en_long(usuario_actual, "Entrada (Inicial)", cant_limpia, nombre_mayus, fecha)
...
def registrar_movimiento(nombre, tipo, cantidad): 
    ...
    # 📝 REGISTRO EXCLUSIVO EN LA PÁGINA 'long'
    usuario_actual = st.session_state.get("usuario", "Desconocido")
    registrar_en_long(usuario_actual, tipo, cant_mov, nombre, fecha_hoy)
            ...
            if not error:
                ...
                for fila in tabla:
                    ...
                    if nueva_cantidad != cant_anterior:
                        ...
                        # 📝 REGISTRO EXCLUSIVO EN LA PÁGINA 'long' POR EDICIÓN
                        usuario_actual = st.session_state.get("usuario", "Desconocido")
                        registrar_en_long(usuario_actual, tipo_mov, abs(diferencia), nuevo_nombre, fecha_hoy)
