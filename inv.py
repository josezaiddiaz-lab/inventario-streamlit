...
# Agregamos el logo oficial en la barra lateral usando tu archivo JPG real
st.sidebar.image("LOGOGCM.jpeg", width=150)
...
def registrar_en_long(usuario, accion, producto, cantidad, fecha):
    """Registra los movimientos exclusivamente en la pestaña 'long' de Google Sheets con orden: USUARIO, ACCION, PRODUCTO, CANTIDAD, FECHA."""
    try:
...
def registrar_producto(nombre, cantidad, fecha, almacen): 
    ...
    # 📝 REGISTRO EXCLUSIVO EN LA PÁGINA 'long' (Producto antes que Cantidad)
    usuario_actual = st.session_state.get("usuario", "Desconocido")
    registrar_en_long(usuario_actual, "Entrada (Inicial)", nombre_mayus, cant_limpia, fecha)
...
def registrar_movimiento(nombre, tipo, cantidad): 
    ...
    # 📝 REGISTRO EXCLUSIVO EN LA PÁGINA 'long' (Producto antes que Cantidad)
    usuario_actual = st.session_state.get("usuario", "Desconocido")
    registrar_en_long(usuario_actual, tipo, nombre, cant_mov, fecha_hoy)
            ...
            if not error:
                ...
                for fila in tabla:
                    ...
                    if nueva_cantidad != cant_anterior:
                        ...
                        # 📝 REGISTRO EXCLUSIVO EN LA PÁGINA 'long' (Producto antes que Cantidad)
                        usuario_actual = st.session_state.get("usuario", "Desconocido")
                        registrar_en_long(usuario_actual, tipo_mov, nuevo_nombre, abs(diferencia), fecha_hoy)
