...
def cargar_inventario_dataframe(): 
    ...
    try:
        ...
        # Compatibilidad si existían nombres antiguos
        if "CANTIDAD_ACTUAL" in df.columns and "CANTIDAD_TOTAL" not in df.columns:
            df["CANTIDAD_TOTAL"] = df["CANTIDAD_ACTUAL"]
        if "ABIERTOS" in df.columns and "CANTIDAD_ABIERTOS" not in df.columns:
            df["CANTIDAD_ABIERTOS"] = df["ABIERTOS"]
        return df
    except Exception:
        return pd.DataFrame(columns=["PRODUCTO", "CANTIDAD_TOTAL", "CANTIDAD_ABIERTOS", "ALMACEN", "FECHA_INGRESO"])
...
# ----------MOVIMIENTO DE STOCK---------- 
def registrar_movimiento(nombre, tipo_movimiento, cantidad_ingresada, abiertos_ingresados, descripcion=""): 
    df_productos = cargar_inventario_dataframe()
    ...
    cant_mov = limpiar_entero(cantidad_ingresada)
    abiertos_mov = limpiar_entero(abiertos_ingresados)
    ...
    tipo_completo = f"{tipo_movimiento}"
    
    registrar_en_historial(nombre, tipo_completo, cant_mov, fecha_hoy, f"{descripcion} (Abiertos afectados: {abiertos_mov})")
    usuario_actual = st.session_state.get("usuario", "sistema")
    registrar_en_hoja_long(usuario_actual, tipo_completo, nombre, cant_mov, fecha_hoy)
    
    st.success(f"Movimiento de {tipo_movimiento.lower()} registrado con éxito.") 
...
# --------MOVIMIENTOS--------
    ...
    if df_productos.empty: 
        ...
        cantidad_ingresada = st.number_input("Cantidad total a mover", min_value=1, step=1) 
        abiertos_ingresados = st.number_input("Cantidad de abiertos a afectar", min_value=0, step=1, value=0)
        
        descripcion = st.text_input("Descripción (Opcional)", placeholder="Escribe un detalle o motivo opcional...")
        
        if st.button("Registrar Movimiento"): 
            registrar_movimiento(nombre, tipo_mov, cantidad_ingresada, abiertos_ingresados, descripcion)
            st.rerun()
    ...
    if df_productos.empty: 
        ...
        for _, row in df_productos.iterrows(): 
            ...
            lista.append({ 
                ...
                "PRODUCTO": nombre, 
                "CANTIDAD_TOTAL": limpiar_entero(row["CANTIDAD_TOTAL"]), 
        ...
        tabla = st.data_editor(
            ...
            column_config={
                ...
                "CANTIDAD_TOTAL": st.column_config.NumberColumn(label="CANTIDAD TOTAL", alignment="center", format="%d"),
                "CANTIDAD_ABIERTOS": st.column_config.NumberColumn(label="ABIERTOS", alignment="center", format="%d"),
        ...
        if tabla != lista:
            ...
            for fila in tabla:
                ...
                if nuevo_nombre in nombres:
                    ...
                    error = True
                    break
                if fila["CANTIDAD_ABIERTOS"] > fila["CANTIDAD_TOTAL"]:
            ...
            if not error:
                ...
                for fila in tabla:
                    nuevos_productos.append({
                        ...
                        "CANTIDAD_TOTAL": limpiar_entero(fila["CANTIDAD_TOTAL"]),
                        "CANTIDAD_ABIERTOS": limpiar_entero(fila["CANTIDAD_ABIERTOS"]),
