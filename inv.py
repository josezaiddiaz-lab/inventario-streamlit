...
def registrar_en_historial(producto, tipo, cantidad, fecha, descripcion=""):
    """Registra los movimientos en la pestaña 'Historial' agregando una fila sin borrar lo anterior."""
    try:
        spreadsheet_url = st.secrets["connections"]["gsheets"]["spreadsheet"]
        df_historial = conn.read(spreadsheet=spreadsheet_url, worksheet="Historial", ttl=0)
    ...
    try:
        conn.update(spreadsheet=spreadsheet_url, worksheet="Historial", data=df_historial_actualizado.astype(str))
    except Exception as e:
        st.warning(f"No se pudo actualizar el historial en Google Sheets. Detalle: {e}")
...
def registrar_movimiento(nombre, tipo_movimiento, estado, cantidad, descripcion=""): 
    ...
    # Lógica de actualización según si es Abierto o Cerrado
...
# --------MOVIMIENTOS--------
    ...
    if df_productos.empty: 
        ...
        # Selección independiente de Tipo y Estado
    ...
    if df_productos.empty: 
        ...
        tabla = st.data_editor(
            ...
            column_config={
                ...
                "CANTIDAD_ACTUAL": st.column_config.NumberColumn(
                    label="CANTIDAD",
                    alignment="center",
                    format="%d"
                ),
