...
# --------EXPORTAR REPORTE A PDF (CORRECCIÓN DEFINITIVA)-------- 
def exportar_pdf(): 
    inventario = cargar_inventario() 
    pdf = FPDF() 
    pdf.add_page() 
...
# ----------VER E INFORMAR INVENTARIO (MODIFICABLE Y VALIDADO)-------- 
if menu == "Ver Inventario": 
    ...
    if not inventario: 
        ...
        # VALIDACIÓN ANTIDUPLICADOS: Verifica cambios antes de guardar
        if tabla_editada != lista_productos:
            nuevo_inventario = {}
            error_duplicado = False
            nombre_duplicado = ""
            
            # 1. Validar que ningún nombre nuevo choque con otro nombre en la misma tabla editada
            nombres_vistos = set()
            for fila in tabla_editada:
                nuevo_nombre = fila["Producto"].strip()
                id_original = fila["Producto Original"]
                
                # Si el usuario cambió el nombre y el nuevo ya existe en el inventario general actual
                # (Y no pertenece al mismo producto que se está editando)
                if nuevo_nombre in inventario and nuevo_nombre != id_original:
                    error_duplicado = True
                    nombre_duplicado = nuevo_nombre
                    break
                
                # Evitar que escriba dos nombres idénticos dentro de la misma edición de la tabla
            ...
            # 2. Si hay error, muestra advertencia roja y frena el guardado
            if error_duplicado:
                st.error(f"❌ Error: El producto '{nombre_duplicado}' ya se encuentra registrado en la lista. No se guardaron los cambios.")
            else:
                # Si todo está bien, procede a estructurar el nuevo diccionario
                for fila in tabla_editada:
                    id_original = fila["Producto Original"]
                    nuevo_nombre = fila["Producto"].strip()
