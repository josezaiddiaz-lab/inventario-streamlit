...
# 🎨 CSS PERSONALIZADO PARA AJUSTAR TABLAS
st.markdown("""
    <style>
    /* Ocultar completamente la barra superior (HUD) de Streamlit */
...
# -------- BARRA LATERAL (Menú, usuario, actualizar y salir) ----------
with st.sidebar:
...
# -------- CABECERA SUPERIOR (Título grande centrado y logo a la derecha) ---------- 
col_title, col_logo = st.columns([5, 1])

with col_title:
    st.markdown("<h1 style='text-align: center;'>Control de Inventario GCM</h1>", unsafe_allow_html=True)

with col_logo:
    st.image("LOGOGCM.jpeg", width=120)
...
def registrar_producto(nombre, cantidad_total, cantidad_abiertos, fecha, almacen): 
    ...
    registrar_en_historial(nombre_mayus, "Entrada (Inicial)", cant_total_limpia, cant_abiertos_limpia, fecha, f"Inicial - Abiertos: {cant_abiertos_limpia}")
...
# ----------MOVIMIENTO DE STOCK---------- 
def registrar_movimiento(nombre, tipo_movimiento, cantidad, abiertos_afectados=0, descripcion=""): 
    df_productos = cargar_inventario_dataframe()
    ...
    registrar_en_historial(nombre, tipo_movimiento, cant_mov, abiertos_mov, fecha_hoy, f"{descripcion} (Abiertos: {abiertos_mov})")
    usuario_actual = st.session_state.get("usuario", "sistema")
    registrar_en_hoja_long(usuario_actual, tipo_movimiento, nombre, cant_mov, fecha_hoy)
...
def exportar_pdf_pedidos(): 
    ...
    for _, row in df_productos.iterrows(): 
        ...
        if tot <= 1: 
            ...
            pdf.cell(w_ab, 8, str(limpiar_entero(row["CANTIDAD ABIERTOS"])), 1, 0, 'C') 
    ...
    if df_productos.empty: 
        ...
        if st.button("Registrar Movimiento"): 
            if not nombre:
                ...
                registrar_movimiento(nombre, tipo_mov, cantidad, abiertos_afectados, descripcion)
                st.rerun()
