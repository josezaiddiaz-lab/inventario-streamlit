...
def exportar_pdf_pedidos(): 
    ...
    pdf.set_font("Arial", 'B', 16) 
    pdf.cell(0, 10, "Reporte de Productos por Agotarse".encode('latin-1', 'replace').decode('latin-1'), ln=True, align='C') 
    pdf.ln(10) 
...
menu = st.sidebar.selectbox(
    ...
    ["Registrar Producto", "Registrar Movimiento", "Ver Inventario", "Solicitar Pedidos"]
)
...
# --------SOLICITAR PEDIDOS--------
    ...
    if not productos_bajos:
        ...
        st.error(f"⚠️ Atención: Tienes {len(productos_bajos)} producto(s) que requieren un pedido urgente.")
