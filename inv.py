...
def registrar_en_long(usuario, accion, producto, cantidad, fecha, descripcion=""):
    """Registra los movimientos exclusivamente en la pestaña 'long' de Google Sheets con orden: USUARIO, ACCION, PRODUCTO, CANTIDAD, DESCRIPCION, FECHA."""
    try:
        df_long = conn.read(worksheet="long", ttl=0)
        if df_long is None or df_long.empty:
            df_long = pd.DataFrame(columns=["USUARIO", "ACCION", "PRODUCTO", "CANTIDAD", "DESCRIPCION", "FECHA"])
    except Exception:
        df_long = pd.DataFrame(columns=["USUARIO", "ACCION", "PRODUCTO", "CANTIDAD", "DESCRIPCION", "FECHA"])
...
def registrar_producto(nombre, cantidad, fecha, almacen): 
    ...
    # 📝 REGISTRO EXCLUSIVO EN LA PÁGINA 'long'
    usuario_actual = st.session_state.get("usuario", "Desconocido")
    registrar_en_long(usuario_actual, "Entrada (Inicial)", nombre_mayus, cant_limpia, fecha, "Registro inicial de producto")
...
def registrar_movimiento(nombre, tipo, cantidad, descripcion=""): 
    ...
    # 📝 REGISTRO EXCLUSIVO EN LA PÁGINA 'long' (Incluyendo la descripción opcional)
    usuario_actual = st.session_state.get("usuario", "Desconocido")
    registrar_en_long(usuario_actual, tipo, nombre, cant_mov, fecha_hoy, descripcion)
...
# --------MOVIMIENTOS--------
    ...
    if df_productos.empty: 
        ...
        # NUEVO APARTADO: Descripción opcional después de cantidad
            ...
            if not error:
                ...
                for fila in tabla:
                    ...
                    if nueva_cantidad != cant_anterior:
                        ...
                        # 📝 REGISTRO EXCLUSIVO EN LA PÁGINA 'long'
                        usuario_actual = st.session_state.get("usuario", "Desconocido")
                        registrar_en_long(usuario_actual, tipo_mov, nuevo_nombre, abs(diferencia), fecha_hoy, "Ajuste manual desde inventario")
