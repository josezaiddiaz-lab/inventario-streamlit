...
# 🎨 CSS PERSONALIZADO PARA QUE EL TEXTO DE LAS TABLAS NO SE SALGA DE LA CELDA
st.markdown("""
    <style>
    /* Ajuste para que las celdas de las tablas muestren todo el texto en varias líneas */
...
def registrar_en_historial(producto, tipo, cantidad, fecha, descripcion=""):
    """Registra los movimientos directamente en la pestaña 'Historial' con las columnas de tu tabla."""
    try:
...
def registrar_en_hoja_long(usuario, accion, producto, cantidad, fecha):
    """Registra los movimientos exclusivamente en la pestaña 'long' de Google Sheets."""
    try:
...
def registrar_producto(nombre, cantidad, abiertos, fecha, almacen): 
    ...
    abiertos_limpios = limpiar_entero(abiertos)
    ...
    registrar_en_hoja_long(usuario_actual, "Entrada (Inicial)", nombre_mayus, cant_limpia, fecha)
    
    return True

# ----------MOVIMIENTO DE STOCK---------- 
def registrar_movimiento(nombre, tipo, cantidad, abiertos_mov, descripcion=""): 
    df_productos = cargar_inventario_dataframe()
    ...
    abiertos_actual = limpiar_entero(df_productos.loc[idx, "ABIERTOS"])
    
    cant_mov = limpiar_entero(cantidad)
    abiertos_mov_limpio = limpiar_entero(abiertos_mov)
    ...
    if tipo == "Entrada": 
        # Se acumula la cantidad total y también se suman los abiertos al total de la cantidad
        nueva_cant = cant_actual + cant_mov + abiertos_mov_limpio
        nuevo_abiertos = abiertos_actual + abiertos_mov_limpio
    else: 
        nueva_cant = cant_actual - cant_mov
        nuevo_abiertos = max(0, abiertos_actual - abiertos_mov_limpio)
        
    df_productos.loc[idx, "CANTIDAD_ACTUAL"] = nueva_cant
    df_productos.loc[idx, "ABIERTOS"] = nuevo_abiertos
    conn.update(worksheet="Productos", data=df_productos.astype(str))
    
    fecha_hoy = datetime.now().strftime('%Y-%m-%d')
    
    # Registramos el movimiento total (cantidad + abiertos sumados para el historial)
    cantidad_total_movimiento = cant_mov + abiertos_mov_limpio if tipo == "Entrada" else cant_mov
    
    desc_completa = f"{descripcion} (Abiertos mod: {abiertos_mov_limpio})" if abiertos_mov_limpio > 0 else descripcion
    registrar_en_historial(nombre, tipo, cantidad_total_movimiento, fecha_hoy, desc_completa)
    
    usuario_actual = st.session_state.get("usuario", "sistema")
    registrar_en_hoja_long(usuario_actual, tipo, nombre, cantidad_total_movimiento, fecha_hoy)
...
if menu == "Registrar Producto": 
    ...
    if "abiertos" not in st.session_state:
        st.session_state.abiertos = 0
    ...
    def procesar_y_limpiar():
        ...
        if nom.strip():
            exito = registrar_producto(nom, cant, abiertos, fecha_str, alm)
            if exito:
                st.session_state.nombre = ""
                st.session_state.cantidad = 0
                st.session_state.abiertos = 0
    ...
    st.number_input("Cantidad Abiertos", min_value=0, step=1, key="abiertos")
...
# --------MOVIMIENTOS--------
    ...
    if df_productos.empty: 
        ...
        # 🔄 ORDEN CAMBIADO: Primero cantidad y enseguida abiertos
        cantidad = st.number_input("Cantidad", min_value=0, step=1, value=0) 
        abiertos_mov = st.number_input("Cantidad Abiertos", min_value=0, step=1, value=0)
        
        descripcion = st.text_input("Descripción (Opcional)", placeholder="Escribe un detalle o motivo opcional...")
        
        if st.button("Registrar Movimiento"): 
            if cantidad == 0 and abiertos_mov == 0:
                st.warning("Debes ingresar al menos una cantidad o un abierto.")
            else:
                registrar_movimiento(nombre, tipo, cantidad, abiertos_mov, descripcion)
                st.rerun()

# --------VER INVENTARIO--------
elif menu == "Ver Inventario": 
    st.subheader("Reporte General de Inventario") 
    st.caption("Puedes corregir nombre, cantidad, abiertos, almacén y fecha de ingreso.")
            ...
            if not error:
                ...
                for fila in tabla:
                    ...
                    if nueva_cantidad != cant_anterior:
                        ...
                        registrar_en_historial(nuevo_nombre, tipo_mov, abs(diferencia), fecha_hoy, "Ajuste manual desde inventario")
                        
                        usuario_actual = st.session_state.get("usuario", "sistema")
                        registrar_en_hoja_long(usuario_actual, tipo_mov, nuevo_nombre, abs(diferencia), fecha_hoy)
