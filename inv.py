...
# --------EXPORTAR REPORTE GENERAL A PDF (AJUSTADO CONTRA DESBORDAMIENTOS)-------- 
def exportar_pdf(): 
    df_productos = cargar_inventario_dataframe()
    ...
    pdf.cell(70, 10, "PRODUCTO", 1, 0, 'L') 
    pdf.cell(30, 10, "CANTIDAD", 1, 0, 'C') 
    pdf.cell(30, 10, "ALMACEN", 1, 0, 'C') 
    pdf.cell(50, 10, "FECHA INGRESO", 1, 1, 'C') 
    ...
    for _, row in df_productos.iterrows(): 
        ...
        # Calcular altura según el texto de PRODUCTO
        y_start = pdf.get_y()
        x_start = pdf.get_x()
        
        # Imprimir producto con multi_cell para envolver texto largo
        pdf.multi_cell(70, 8, prod_nombre, border=1, align='L')
        y_end = pdf.get_y()
        h_row = y_end - y_start
        
        # Posicionar celdas restantes para mantener la fila alineada
        pdf.set_xy(x_start + 70, y_start)
        pdf.cell(30, h_row, cant, 1, 0, 'C') 
        pdf.cell(30, h_row, almacen, 1, 0, 'C') 
        pdf.cell(50, h_row, fecha_ing, 1, 1, 'C') 
        
    return bytes(pdf.output(dest='S'))

# --------EXPORTAR REPORTE DE PEDIDOS A PDF (AJUSTADO CONTRA DESBORDAMIENTOS)-------- 
def exportar_pdf_pedidos(): 
    ...
    pdf.set_font("Arial", 'B', 10) 
    pdf.cell(100, 10, "PRODUCTO", 1, 0, 'L') 
    pdf.cell(40, 10, "CANTIDAD", 1, 0, 'C') 
    pdf.cell(40, 10, "ALMACEN", 1, 1, 'C') 
    ...
    for _, row in df_productos.iterrows(): 
        ...
        if cant <= 1: 
            ...
            pdf.multi_cell(100, 8, prod_nombre, border=1, align='L')
            y_end = pdf.get_y()
            h_row = y_end - y_start
            
            pdf.set_xy(x_start + 100, y_start)
            pdf.cell(40, h_row, str(cant), 1, 0, 'C') 
            pdf.cell(40, h_row, str(row["ALMACEN"]), 1, 1, 'C') 
            
    return bytes(pdf.output(dest='S'))

# --------EXPORTAR A EXCEL (CON ANCHO AUTO-AJUSTABLE Y WRAP TEXT)-------- 
def exportar_excel_general(df, titulo_pestana="Inventario"):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        for col_idx, col_name in enumerate(df.columns, 1):
            ...
            # Calcular ancho máximo de texto en la columna
            ...
            # Ajustar ancho de columna dinámicamente con margen de espacio
...
# --------VER INVENTARIO--------
    ...
    if df_productos.empty: 
        ...
        # Botones de descarga organizados
