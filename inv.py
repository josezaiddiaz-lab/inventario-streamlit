...
def registrar_producto(nombre, cantidad, fecha): 
    ...
    if nombre_mayus in inventario: 
        st.warning("El producto ya existe in el inventario.") 
        return 
...
if menu == "Registrar Movimiento": 
    ...
    if not productos: 
        ...
        # Buscador inteligente para los productos (Aquí sí se puede escribir)
        nombre = st.selectbox(
        ...
        # CORRECCIÓN: Se cambió st.selectbox por st.radio para bloquear por completo la escritura manual
        tipo = st.radio(
            "Tipo de movimiento", 
            options=["Entrada", "Salida"],
            horizontal=True  # Hace que aparezcan uno al lado del otro, ideal para celular
        ) 
