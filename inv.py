...
# ----------NUEVA OPCIÓN: SOLICITAR PEDIDOS (SOLUCIÓN MATEMÁTICA)-------- 
if menu == "Solicitar Pedidos": 
    ...
    productos_bajos = [] 
    for nombre, datos in inventario.items(): 
        # Solución definitiva: evalúa si la cantidad es menor o igual a 1 (0 o 1)
        if datos["cantidad"] <= 1: 
    ...
    if not productos_bajos: 
        ...
        st.error(f" Atención: Tienes {len(productos_bajos)} producto(s) que requieren un pedido.") 
