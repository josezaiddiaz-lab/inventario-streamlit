...
st.set_page_config(
    page_title="Control de Inventario GCM",[cite: 2]
    page_icon="📦",
    layout="centered",
...
def registrar_en_historial(producto, tipo, cantidad, abiertos, fecha, descripcion=""):
    ...
    nuevo_registro = pd.DataFrame([{
        ...
        "ABIERTOS": limpiar_entero(abiertos), # <--- CORREGIDO AQUÍ PARA USAR EL VALOR REAL DE ABIERTOS
        "FECHA": fecha,
...
def registrar_movimiento(nombre, tipo_movimiento, estado, cantidad, abiertos_afectados=0, descripcion=""): 
    ...
    registrar_en_historial(nombre, tipo_completo, cant_mov, abiertos_mov, fecha_hoy, f"{descripcion} (Abiertos: {abiertos_mov})")
    usuario_actual = st.session_state.get("usuario", "sistema")
...
# --------INTERFAZ STREAMLIT---------- 
st.title("Control de Inventario GCM")[cite: 2]
